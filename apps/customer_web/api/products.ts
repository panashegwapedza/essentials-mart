...
export default async function productsHandler(req:any,res:any){
 ...
 try{
  ...
  const storeId=stores[0]?.id;if(!storeId)return res.status(500).json({error:{message:'MAIN store is not configured.'}});
  const id=typeof req.query?.id==='string'?req.query.id:null;
  const products=await supabase<ProductRow[]>('products',id?'select=id,name,category,product_family,brand,variant_label,size_label,image_url,price,currency,is_active&id=eq.'+encodeURIComponent(id)+'&limit=1':'select=id,name,category,product_family,brand,variant_label,size_label,image_url,price,currency,is_active&is_active=eq.true&order=name.asc');
  ...
  const map=(p:ProductRow)=>({id:p.id,name:p.name,category:p.category??'Essentials',productFamily:p.product_family??undefined,brand:p.brand??undefined,variantLabel:p.variant_label??undefined,sizeLabel:p.size_label??undefined,imageUrl:p.image_url??undefined,price:{amountMinor:Math.round(Number(p.price)*100),currency:p.currency},available:Boolean(p.is_active)&&(stock.get(p.id)??0)>0,stockQuantity:stock.get(p.id)??0});
  if(id){if(!products[0])return res.status(404).json({error:{message:'Product not found.'}});return res.status(200).json(map(products[0]));}
  return res.status(200).setHeader('Cache-Control','no-store').json({products:products.map(map)});
 }catch(e){return res.status(500).json({error:{message:e instanceof Error?e.message:'Catalogue unavailable.'}});}
}