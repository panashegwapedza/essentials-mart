...
export function evaluateBasketReadiness(
  ...
  if (!readiness || !Array.isArray(readiness.items) || readiness.items.length === 0) {
    return { ready: false, reason: 'Basket readiness could not be verified.' };
  }
  ...
  const blockingItem = readiness.items.find(
    (item: any) =>
      !item ||
      typeof item !== 'object' ||
      item.status !== 'healthy' ||
  ...
  return { ready: true, reason: 'Basket is ready for checkout.' };
}
