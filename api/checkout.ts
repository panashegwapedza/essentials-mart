...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!['pickup', 'standard', 'express'].includes(deliveryMethod)) {
      return json(res, 400, { error: { code: 'INVALID_DELIVERY_METHOD', message: 'Select a valid delivery method before checkout.' } });
    }

    const idempotencyKey = typeof req.headers?.['idempotency-key'] === 'string' ? req.headers['idempotency-key'] : undefined;
    const order = await commerce.checkout(user, deliveryMethod, idempotencyKey);
    return json(res, 201, orderDto(order));
  } catch (error: any) {
    const status = error?.code === 'NOT_FOUND' ? 404
      : error?.code === 'PRODUCT_UNAVAILABLE' || error?.code === 'INSUFFICIENT_STOCK' ? 409
      : error?.code === 'BASKET_EMPTY' ? 400
      : 500;
    console.error('checkout-api error', error);
    return json(res, status, { error: { code: error?.code ?? 'CHECKOUT_FAILED', message: error instanceof Error ? error.message : 'Checkout failed.' } });
  }
}
