...
export default async function handler(req:any,res:any){
 ...
 try{
  ...
  const body=typeof req.body==='string'?JSON.parse(req.body):(req.body??{}); const method=body?.deliveryMethod; const idempotencyKey=String(req.headers?.['idempotency-key']??req.headers?.['Idempotency-Key']??'').trim();
  ...
  if(!idempotencyKey||idempotencyKey.length>200)return json(res,400,{error:{code:'INVALID_IDEMPOTENCY_KEY',message:'A valid Idempotency-Key is required.'}});
