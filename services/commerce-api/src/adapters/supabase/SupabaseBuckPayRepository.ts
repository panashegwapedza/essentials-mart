...
function moneyFromRow(row: any): Money { return { amountMinor: Math.round(Number(row.amount ?? row.balance ?? 0) * 100), currency: row.currency }; }
...
async function customerDbId(customerId: string): Promise<string> {
  ...
  const created = await supabaseRest<any[]>("customers", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ external_customer_id: customerId }) });
...
export class SupabaseBuckPayRepository implements BuckPayRepository {
  async getAccount(customerId: string): Promise<BuckPayAccount> {
    ...
    if (!rows[0]) {
      const created = await supabaseRest<any[]>("buckpay_accounts", { method: "POST", headers: { Prefer: "return=representation,resolution=merge-duplicates" }, body: JSON.stringify({ customer_id: dbCustomerId, currency: "ZiG", balance: 0, status: "active" }) });
      if (!created[0]) throw new Error("Supabase BuckPay account creation returned no row.");
      return { customerId, balance: { amountMinor: Math.round(Number(created[0].balance) * 100), currency: created[0].currency }, status: created[0].status === "active" ? "active" : "suspended" };
    }
    return { customerId, balance: { amountMinor: Math.round(Number(rows[0].balance) * 100), currency: rows[0].currency }, status: rows[0].status === "active" ? "active" : "suspended" };
  }
  ...
  async appendTransaction(transaction: BuckPayTransaction): Promise<BuckPayTransaction> {
    const result = await supabaseRpc<any>("buckpay_append_transaction", { p_customer_external_id: transaction.customerId, p_transaction_id: transaction.id, p_transaction_type: transaction.type, p_amount: transaction.amount.amountMinor / 100, p_currency: transaction.amount.currency, p_reference: transaction.reference, p_created_at: transaction.createdAt });
    const payload = result?.transaction ?? result;
    return { id: payload.id, customerId: payload.customerId ?? transaction.customerId, type: payload.type ?? transaction.type, amount: { amountMinor: Math.round(Number(payload.amount) * 100), currency: payload.currency ?? transaction.amount.currency }, reference: payload.reference ?? transaction.reference, createdAt: payload.createdAt ?? transaction.createdAt };
  }
}
