...
async function rest<T>(table: string, init: RequestInit = {}): Promise<T> {
...
async function ensureCustomer(externalCustomerId: CustomerId): Promise<string> {
  const found = await rest<any[]>(`customers?select=id&external_customer_id=eq.${encodeURIComponent(externalCustomerId)}&limit=1`);
...
export class SupabaseProductRepository implements ProductRepository {
  async list(): Promise<Product[]> {
    const rows = await rest<any[]>("products?select=id,name,price,currency,is_active&is_active=eq.true&order=name.asc");
    return rows.map(product);
  }

  async getById(id: ProductId): Promise<Product | null> {
    const rows = await rest<any[]>(`products?select=id,name,price,currency,is_active&id=eq.${encodeURIComponent(id)}&limit=1`);
...
export class SupabaseBasketRepository implements BasketRepository {
  async getByCustomerId(customerId: CustomerId): Promise<Basket | null> {
    const customerRows = await rest<any[]>(`customers?select=id,external_customer_id&external_customer_id=eq.${encodeURIComponent(customerId)}&limit=1`);
    const customer = customerRows[0];
    if (!customer) return null;
    const baskets = await rest<any[]>(`baskets?select=id,currency,status,updated_at,customers!inner(external_customer_id)&customer_id=eq.${customer.id}&status=eq.active&order=updated_at.desc&limit=1`);
    const basket = baskets[0];
    if (!basket) return null;
    const items = await rest<any[]>(`basket_items?select=product_id,quantity,unit_price&basket_id=eq.${basket.id}&order=created_at.asc`);
  ...
  async save(basket: Basket): Promise<void> {
    ...
    await rest(`basket_items?basket_id=eq.${basket.id}`, { method: "DELETE" });
    if (basket.lines.length) {
      await rest("basket_items", {
        method: "POST",
...
export class SupabaseOrderRepository implements OrderRepository {
  ...
  private async load(id: OrderId): Promise<Order | null> {
    const orders = await rest<any[]>(`orders?select=id,customer_id,currency,total,status,customers!inner(external_customer_id)&id=eq.${encodeURIComponent(id)}&limit=1`);
    const row = orders[0];
    if (!row) return null;
    const items = await rest<any[]>(`order_items?select=product_id,quantity,unit_price&order_id=eq.${encodeURIComponent(id)}&order=created_at.asc`);
    return {
      id: row.id,
...
export class SupabaseCheckoutTransaction implements CheckoutTransaction {
  async run<T>(work: () => Promise<T>): Promise<T> {
    // Repository writes are durable. Atomic multi-table checkout will be promoted to a Postgres RPC in the next transaction-hardening step.
    return work();
  }
}
