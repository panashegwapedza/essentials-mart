...
export function runAISociety(request: IntelligenceRequest): IntelligenceResponse {
  if (request.capability === 'search-understanding') {
    return {
      ...understandSearchQuery(request.query, request.vocabulary),
      trace: { agentId: 'catalogue-search-agent.v1', authority: 'interpretation-only' },
    };
  }
  if (request.capability === 'household-recommendations') {
    return generateHouseholdRecommendations(request.signals, request.catalogue);
  }
  if (request.capability === 'household-predictions') {
    return generateHouseholdPredictions(request.signals);
  }
