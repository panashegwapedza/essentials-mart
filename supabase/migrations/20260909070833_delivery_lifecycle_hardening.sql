...
begin
  ...
  if v_delivery.status='pending' and p_status in ('preparing','cancelled') then v_allowed := true;
  elsif v_delivery.status='preparing' and p_status in ('ready_for_pickup','out_for_delivery','failed','cancelled') then v_allowed := true;
  elsif v_delivery.status='ready_for_pickup' and p_status in ('delivered','failed','cancelled') then v_allowed := true;
  elsif v_delivery.status='out_for_delivery' and p_status in ('delivered','failed','cancelled') then v_allowed := true;
  elsif v_delivery.status='failed' and p_status in ('preparing','cancelled') then v_allowed := true;
  end if;
  ...
  if not v_allowed then raise exception 'Invalid delivery transition: % -> %', v_delivery.status, p_status; end if;
  ...
  insert into public.delivery_status_history(delivery_id,order_id,customer_id,from_status,to_status,tracking_reference,changed_at)
  values(v_delivery.id,v_delivery.order_id,v_delivery.customer_id,case when v_delivery.status='pending' then null else null end,p_status,v_delivery.tracking_reference,now());
