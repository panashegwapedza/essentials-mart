create table if not exists public.checkout_idempotency_keys (
  ...
  constraint checkout_idempotency_key_length check (char_length(idempotency_key) between 1 and 200),
  constraint checkout_idempotency_fingerprint_length check (char_length(request_fingerprint) between 1 and 512),
  constraint checkout_idempotency_customer_key_unique unique (customer_id, idempotency_key),
  constraint checkout_idempotency_order_unique unique (order_id)
);
...
begin
  ...
  for v_item in select * from jsonb_to_recordset(v_items) as x("productId" uuid,"quantity" integer) loop
    update public.inventory set quantity=quantity-v_item."quantity",updated_at=now() where product_id=v_item."productId";
  end loop;
  for v_item in select * from jsonb_to_recordset(v_items) as x("productId" uuid,"productName" text,"sku" text,"quantity" integer,"unitPrice" numeric,"lineTotal" numeric) loop
    insert into public.order_items(order_id,product_id,product_name,sku,quantity,unit_price,line_total) values(p_order_id,v_item."productId",v_item."productName",v_item."sku",v_item."quantity",v_item."unitPrice",v_item."lineTotal");
  end loop;
  ...
  if p_idempotency_key is not null then
    update public.checkout_idempotency_keys set order_id=p_order_id,updated_at=now() where customer_id=v_customer_id and idempotency_key=btrim(p_idempotency_key);
  end if;
