...
alter table public.orders add constraint orders_delivery_method_check
  check (delivery_method = any (array['pickup','standard','express']));
...
begin
  ...
  for v_item in
    ...
    from public.basket_items bi join public.products p on p.id = bi.product_id
    where bi.basket_id = v_basket.id order by bi.created_at asc for update of bi
  loop
  ...
  v_order_total := v_subtotal + v_delivery_fee;
  ...
  values (p_order_id, p_order_id::text, v_customer_id, v_basket.id, 'placed', 'unpaid', v_currency, v_subtotal, p_delivery_method, v_delivery_fee, v_order_total)
  ...
  loop
    ...
    values (p_order_id, v_item."productId", v_item."productName", v_item."sku", v_item."quantity", v_item."unitPrice", v_item."lineTotal") on conflict do nothing;
  end loop;
