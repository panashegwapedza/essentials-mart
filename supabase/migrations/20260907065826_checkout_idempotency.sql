...
begin
  ...
  if v_order.id is not null then
    ...
    return jsonb_build_object('id',v_order.id,'customerId',p_customer_external_id,'status',v_order.status,'total',jsonb_build_object('amountMinor',round(v_order.total*100)::integer,'currency',v_order.currency),'lines',v_items,'idempotent',true);
  end if;
  ...
  insert into public.orders(id,order_number,customer_id,basket_id,status,payment_status,currency,subtotal,total) values(p_order_id,p_order_id::text,v_customer_id,v_basket.id,'placed','unpaid',v_currency,v_order_total,v_order_total);
  ...
  return jsonb_build_object('id',v_order.id,'customerId',p_customer_external_id,'status',v_order.status,'total',jsonb_build_object('amountMinor',round(v_order.total*100)::integer,'currency',v_order.currency),'lines',v_items,'idempotent',false);
end;
$$;
